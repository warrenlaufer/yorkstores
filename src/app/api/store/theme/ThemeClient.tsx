...
export default function ThemeClient({
  ...
  if (!contrastOk(btnTextColor, theme.primaryColor)) warnings.push('Button text vs Primary Color contrast is too low')
  if (!contrastOk(accentTextColor, theme.accentColor)) warnings.push('Accent text contrast is too low')
    ...
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '1.5rem 1.25rem 3rem', fontFamily: 'var(--font, system-ui)', color: '#EEEEF5' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          ...
          <button onClick={save} disabled={saving} style={{ background: '#FF6B85', color: '#fff', border: 'none', borderRadius: 8, fontFamily: 'var(--font)', fontSize: '0.75rem', fontWeight: 800, padding: '0.4rem 0.9rem', cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: '1.5rem', alignItems: 'flex-start' }}>
        ...
        <div style={{ background: '#0F0F14', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 12, padding: '1rem' }}>
          <div style={{ fontSize: '0.62rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#FF8FA3', marginBottom: '1rem' }}>Colors</div>
          {FIELDS.map(f => (
          ...
          {firstDrop && (
            <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
              <div style={{ fontSize: '0.62rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#FF8FA3', marginBottom: '0.6rem' }}>Embed Code</div>
              <div style={{ background: '#1D1D26', borderRadius: 8, padding: '0.6rem', fontSize: '0.65rem', fontFamily: 'monospace', color: '#9898B0', wordBreak: 'break-all', lineHeight: 1.6 }}>
                {`<iframe src="https://www.yorkstores.com/embed/drop/${firstDrop.id}" width="100%" height="700" frameborder="0" style="border-radius:12px;"></iframe>`}
              </div>
              <button
                onClick={() => navigator.clipboard.writeText(`<iframe src="https://www.yorkstores.com/embed/drop/${firstDrop.id}" width="100%" height="700" frameborder="0" style="border-radius:12px;"></iframe>`)}
                style={{ marginTop: '0.5rem', width: '100%', background: 'rgba(126,200,255,0.1)', color: '#7EC8FF', border: '1px solid rgba(126,200,255,0.3)', borderRadius: 6, fontFamily: 'var(--font)', fontSize: '0.72rem', fontWeight: 700, padding: '0.35rem', cursor: 'pointer' }}
              >
                Copy Embed Code
              </button>
              {drops.length > 1 && (
                <p style={{ fontSize: '0.6rem', color: '#9898B0', marginTop: '0.4rem' }}>
                  Showing code for "{firstDrop.name}". Replace the drop ID for other drops.
                </p>
              )}
            </div>
          )}
        ...
        {/* Live preview */}
        <div style={{ background: '#0F0F14', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 12, overflow: 'hidden' }}>
          <div style={{ padding: '0.6rem 1rem', borderBottom: '1px solid rgba(255,255,255,0.08)', fontSize: '0.62rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#FF8FA3' }}>Live Preview</div>
          <div style={{ background: theme.backgroundColor, padding: '1.25rem', minHeight: 400 }}>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', marginBottom: '1rem' }}>
              ...
              <div>
                <div style={{ fontSize: '0.55rem', fontWeight: 700, color: theme.primaryColor, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 2 }}>Trading Cards</div>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
              {[
                ...
                <div key={s.lbl} style={{ background: theme.cardColor, border: `1px solid ${theme.primaryColor}22`, borderRadius: 8, padding: '0.35rem 0.6rem', textAlign: 'center' }}>
                  <div style={{ fontFamily: 'monospace', fontSize: '0.85rem', fontWeight: 800, color: s.color }}>{s.val}</div>
                  <div style={{ fontSize: '0.5rem', color: theme.textColor + '80', textTransform: 'uppercase', letterSpacing: '0.07em' }}>{s.lbl}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ marginBottom: '1rem' }}>
              <div style={{ height: 4, background: theme.cardColor, borderRadius: 2, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: '40%', background: theme.primaryColor, borderRadius: 2 }} />
              </div>
            </div>

            {/* Mock CTA */}
            <div style={{ background: theme.cardColor, border: `1px solid ${theme.primaryColor}44`, borderRadius: 10, padding: '1rem', marginBottom: '1rem', textAlign: 'center' }}>
              <p style={{ fontSize: '0.7rem', color: theme.textColor + '99', marginBottom: '0.75rem' }}>
                Create a free account to open a box.
              </p>
              <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center' }}>
                <div style={{ background: theme.primaryColor, color: readable(theme.primaryColor), borderRadius: 8, padding: '0.5rem 1.1rem', fontWeight: 800, fontSize: '0.8rem' }}>
                  Open a Box — $25
                </div>
                <div style={{ background: 'transparent', color: theme.textColor, border: `1px solid ${theme.textColor}33`, borderRadius: 8, padding: '0.5rem 1rem', fontWeight: 700, fontSize: '0.8rem' }}>
                  Sign In
                </div>
              </div>
            </div>
