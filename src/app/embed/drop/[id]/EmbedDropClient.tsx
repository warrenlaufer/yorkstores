  ...
  return (
    <div style={{ minHeight: '100vh', background: theme.backgroundColor, color: theme.textColor, fontFamily: 'system-ui, sans-serif', padding: '1.5rem 1.25rem 3rem' }}>
      <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        {drop.logoUrl ? (
          <img src={drop.logoUrl} alt={drop.name} style={{ width: 80, height: 80, objectFit: 'contain', background: theme.cardColor, borderRadius: 12, padding: '0.4rem', flexShrink: 0 }} />
        ) : (
          <div style={{ width: 80, height: 80, background: theme.cardColor, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2.5rem', flexShrink: 0 }}>
            {drop.emoji || '🎁'}
          </div>
        )}
        <div style={{ flex: 1 }}>
          ...
          <h1 style={{ fontSize: '1.5rem', fontWeight: 900, letterSpacing: '-0.02em', marginBottom: '0.2rem', color: theme.textColor }}>{drop.name}</h1>
          <p style={{ fontSize: '0.75rem', color: theme.textColor + '99', marginBottom: '0.75rem' }}>by {drop.owner}</p>
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            <div style={{ background: theme.cardColor, borderRadius: 8, padding: '0.4rem 0.7rem', textAlign: 'center' }}>
              <div style={{ fontFamily: 'monospace', fontSize: '0.95rem', fontWeight: 800, color: theme.accentColor }}>${drop.boxPrice}</div>
              <div style={{ fontSize: '0.55rem', color: theme.textColor + '80', textTransform: 'uppercase', letterSpacing: '0.07em', marginTop: 2 }}>Per Box</div>
            </div>
            <div style={{ background: theme.cardColor, borderRadius: 8, padding: '0.4rem 0.7rem', textAlign: 'center' }}>
              <div style={{ fontFamily: 'monospace', fontSize: '0.95rem', fontWeight: 800, color: '#3DD68C' }}>{drop.sellBackPct}%</div>
              <div style={{ fontSize: '0.55rem', color: theme.textColor + '80', textTransform: 'uppercase', letterSpacing: '0.07em', marginTop: 2 }}>Sell Back</div>
            </div>
            <div style={{ background: theme.cardColor, borderRadius: 8, padding: '0.4rem 0.7rem', textAlign: 'center' }}>
              <div style={{ fontFamily: 'monospace', fontSize: '0.95rem', fontWeight: 800, color: '#7EC8FF' }}>{drop.availableBoxes}</div>
              <div style={{ fontSize: '0.55rem', color: theme.textColor + '80', textTransform: 'uppercase', letterSpacing: '0.07em', marginTop: 2 }}>Remaining</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ background: theme.cardColor, borderRadius: 12, padding: '1.25rem', marginBottom: '1.5rem', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '0.6rem', justifyContent: 'center', flexWrap: 'wrap' }}>
          <a
            ...
            target="_top"
            style={{ background: soldOut ? '#2a2a35' : theme.primaryColor, color: soldOut ? 'rgba(255,255,255,0.3)' : btnTextColor, border: 'none', borderRadius: 10, padding: '0.65rem 1.5rem', fontWeight: 800, fontSize: '0.88rem', textDecoration: 'none', display: 'inline-block' }}
          >
            {soldOut ? 'Sold Out' : 'Open a Box - $' + drop.boxPrice}
          </a>
          
          <a href={'https://www.yorkstores.com/signin?redirect=/dashboard/drop/' + drop.id}
            target="_top"
            style={{ background: 'transparent', color: theme.textColor, border: '1px solid ' + theme.textColor + '33', borderRadius: 10, padding: '0.65rem 1.5rem', fontWeight: 700, fontSize: '0.88rem', textDecoration: 'none', display: 'inline-block' }}
          >
            Sign In
          </a>
        </div>
        <p style={{ fontSize: '0.6rem', color: theme.textColor + '55', marginTop: '0.6rem' }}>
          Powered by Yorkstores
        </p>
      </div>

      <div>
        <h2 style={{ fontSize: '0.9rem', fontWeight: 800, marginBottom: '0.2rem', color: theme.textColor }}>What is Inside</h2>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
          {odds.map(item => {
            ...
            return (
              <div key={item.name + item.price} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', background: theme.cardColor, borderRadius: 8, padding: '0.5rem 0.75rem' }}>
                {item.imageUrl ? (
